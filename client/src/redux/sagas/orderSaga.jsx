...
function* addOrderSaga(action) {
    try {
        const response = yield call(axios.post, `${import.meta.env.VITE_LOCALHOST}/order/addOrder`, action.payload);
...
function* getAllOrderSaga() {
    try {
        const response = yield call(axios.get, `${import.meta.env.VITE_LOCALHOST}/order/getAllOrder`);
...
function* getOrderByIdSaga(action) {
    try {
        const response = yield call(axios.get, `${import.meta.env.VITE_LOCALHOST}/order/orderById/${action.payload}`);
...
function* getOrderStatusSaga(action) {
    try {
        const response = yield call(
            axios.get,
            `${import.meta.env.VITE_LOCALHOST}/order/orderStatus/${action.payload.storeid}?status=${
...
function* getOrderDraftSaga(action) {
    try {
        const response = yield call(axios.get, `${import.meta.env.VITE_LOCALHOST}/order/orderDraft/${action.payload}`);
...
function* newOrderSaga(action) {
    try {
        ...
        action.payload.navigate(`/pay-success?si=${response.data.store.id}&oi=${response.data.order.id}`);
    } catch (error) {
...
function* orderStatisticSaga(action) {
    try {
        const response = yield call(axios.get, `${import.meta.env.VITE_LOCALHOST}/order/statistics/${action.payload}`);
