...
import { Card, Input, Button, Form, Table, Space, Radio, Modal, Select, Checkbox, Empty } from 'antd';
import { PlusOutlined, WalletOutlined, BankOutlined, DollarCircleOutlined, HomeOutlined } from '@ant-design/icons';
...
import { getOrderDraftStart, newOrderStart } from '../../../redux/slices/orderSlice';
...
const CheckoutPage = () => {
    ...
    const { additionalAddresses } = useSelector((state) => state.additionalAddresses);
    const additionalAddressList = Array.isArray(additionalAddresses.addresses) ? additionalAddresses.addresses : [];
    useEffect(() => {
        dispatch(fetchAdditionalAddressesStart(getToken().id));
    }, [dispatch]);

    const [selectedAddress, setSelectedAddress] = useState(mainAddress?.address?.id);
    useEffect(() => {
    ...
    return orderDraft ? (
        <div style={{ backgroundColor: '#fff', padding: '40px 150px' }}>
            <Card title="Địa chỉ người nhận" className="mb-4">
                    ...
                    <Form.Item name="address">
                        ...
                        >
                            <Card
                                ...
                                className={`mb-2 ${
                                    selectedAddress === mainAddress?.mainAddress?.id ? 'border-primary' : ''
                                }`}
                            ...
                            >
                                <Radio value={mainAddress?.mainAddress?.id} style={{ display: 'block' }}>
                                    {mainAddress?.mainAddress.address} - {mainAddress?.mainAddress.Ward.name} -{' '}
                            ...
                            {additionalAddressList.map((addr) => (
                                <Card
                                    key={addr.id}
                                    className={`mb-2 ${selectedAddress === addr.id ? 'border-primary' : ''}`}
                                    onClick={() => setSelectedAddress(addr.id)}
                                    style={{
                                        cursor: 'pointer',
                                        border: selectedAddress === addr.id ? '2px solid #1890ff' : '1px solid #d9d9d9',
                                    }}
                                >
                ...
                <Modal title="Thêm địa chỉ mới" open={isModalOpen} onCancel={() => setIsModalOpen(false)} footer={null}>
            ...
            <div className="flex gap-4">
                <Card title="Thông tin người nhận" className="mb-4 flex-1">
                    ...
                    <p className="mb-3">
                        <strong>Địa chỉ:</strong> {selectAddress?.address.address} - {selectAddress?.address.Ward.name}{' '}
