...
import { Steps, Modal, Card, Typography, Row, Col, Divider, Button, Table, Image } from 'antd';
import { PhoneOutlined, HomeOutlined, UserOutlined, DollarOutlined, CreditCardOutlined } from '@ant-design/icons';
...
const OrderStatus = () => {
    ...
    const confirmChange = () => {
        ...
        if (statusSteps[newStatus] === 'Đang đóng gói') {
            socket.emit('sendMessage', {
                message: `Đơn hàng ${id} của bạn đã được phê duyệt và đang đóng gói`,
                accountid: order.accountid,
            });
        } else if (statusSteps[newStatus] === 'Đang vận chuyển') {
            socket.emit('sendMessage', {
            ...
            socket.emit('sendMessage', {
                message: 'Đơn hàng của bạn đã được giao. Hãy viết bình luận đánh giá về sản phẩm',
                accountid: order.accountid,
            });
        }
    };
