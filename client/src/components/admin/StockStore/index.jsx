import { Button, Card, Checkbox, InputNumber, message, Modal, Table, Tag, Typography } from 'antd';
...
const StockStore = () => {
    ...
    return (
        <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
            <Modal title="Nhập kho" open={isModalOpen} onCancel={closeImportModal} onOk={handleImportStock}>
                <p>Sản phẩm: {selectedProduct?.productname}</p>
                <InputNumber min={1} value={importQuantity} onChange={setImportQuantity} placeholder="Nhập số lượng" />
                <Checkbox checked={isConfirmed} onChange={(e) => setIsConfirmed(e.target.checked)}>
                    Tôi xác nhận số lượng nhập vào là chính xác và chịu trách nhiệm nếu có sai sót.
                </Checkbox>
            </Modal>
                ...
                <Card className="shadow-lg p-6 w-96 text-center">
                    {storeAccount?.accountStore?.Store?.status === 'Cần khởi tạo' && (
                        <>
                            ...
                            <Button type="primary" block className="mt-4" onClick={handleInitializeStock}>
                                Khởi tạo kho
                            </Button>
                        </>
                    )}
                    ...
                    {storeAccount?.accountStore?.Store?.status === 'Cần cập nhật' && (
                        <>
                            ...
                            <Text>Có sản phẩm mới từ công ty. Vui lòng cập nhật để tiếp tục.</Text>
                            <Button type="primary" block className="mt-4" onClick={handleUpdateStoreStock}>
                                Cập nhật sản phẩm
                            </Button>
                        </>
                    )}
