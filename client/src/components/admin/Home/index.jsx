...
import { orderMonthStart, orderStatisticStart } from '../../../redux/slices/orderSlice';
import dayjs from 'dayjs';
import { getToken } from '../../../utils/token';
import { getAccountStorebyAccountIdStart } from '../../../redux/slices/storeAccountSlice';
import { fetchProductStaticsStart } from '../../../redux/slices/storeStockSlice';
...
const AdminHome = () => {
    ...
    const orderStatisticList = Array.isArray(orderStatistics) ? orderStatistics : [];
    const { productStatics } = useSelector((state) => state.storeStocks);
    const { orderMonth } = useSelector((state) => state.orders);
    const currentDate = dayjs();

    const [dateRange, setDateRange] = useState([currentDate.subtract(6, 'month'), currentDate]);
    useEffect(() => {
        dispatch(orderStatisticStart(storeAccount?.accountStore?.Store?.id));
        dispatch(fetchProductStaticsStart(storeAccount?.accountStore?.Store?.id));
        dispatch(
            orderMonthStart({
    ...
    return (
        <Container>
                ...
                {orderStatisticList.map((order, index) => {
                    ...
                    return (
                        <Col key={index} span={6}>
                            {' '}
                            <Card bordered style={{ textAlign: 'center', borderTop: `5px solid ${color}` }}>
                                <Statistic
                                    title={order.status}
            ...
            <Card>
                <Bar
                    data={productStatics && productStatics.labels ? productStatics : { labels: [], datasets: [] }}
                    options={{
                        responsive: true,
            ...
            <RangePicker
                ...
                value={dateRange}
                disabledDate={(current) => current && current.isAfter(currentDate, 'month')}
